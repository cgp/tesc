...
Snapshot diagnostics answer whether the 250ms flush occupies the arrival execution loop. `snapshot_timing` annotations measure aggregation (pending sends, histogram merge/reset and diagnostic synchronization), window construction (gauges and histogram clones), complete flush (including snapshot-consumer handoff and interval reset), and output-packet construction (identity, window/diagnostic clones and boxing, excluding queue admission and writer serialization). `snapshot_timing_total` reports the target/step’s snapshot populations with counts and shared percentile support; stderr prints their counts and maxima. All phase-boundary flushes are included as well as periodic ticks. An output packet rejected before construction adds no construction sample; a packet constructed then rejected remains a measured attempt. These durations do not change scheduling or run validity.
