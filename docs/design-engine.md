...
- Aggregation happens in-process: per-worker histograms and counters, handed to a dedicated aggregation thread on the snapshot tick through three preallocated buffer sets. The arrival loop only records pending sends, captures scalar gauges and swaps buffers; merging, resetting, cumulative diagnostics and window construction run on the aggregation thread. The hot path does no locking and no per-request allocation.
...
Snapshot diagnostics measure aggregation-thread work, while arrival wake-to-dispatch measures whether snapshot handoff still delays dispatch. Three reusable buffer sets bound outstanding work and results. When no spare is available, periodic snapshots coalesce without dropping measurements or delaying arrivals; `snapshot_backpressure` reports the coalesced tick count. Phase boundaries and final cancellation finish pending aggregation before advancing or evaluating SLOs. Breakpoint assessments use cumulative metrics and scalar diagnostics from the same captured cutoff; a returned stop decision takes effect when the scheduler receives it. External snapshot and output consumers remain nonblocking. `snapshot_timing` annotations measure aggregation (histogram merge/reset and diagnostic synchronization), window construction (captured gauges and histogram clones), complete flush (including snapshot-consumer handoff and interval reset), and output-packet construction (identity, shared window ownership and diagnostic boxing, excluding queue admission and writer serialization). `snapshot_timing_total` reports the target/step’s snapshot populations with counts and shared percentile support; stderr prints their counts and maxima. The paired statistic adds complete flush and output-packet construction from the same snapshot; its maximum does not add unrelated component maxima. All phase-boundary flushes are included as well as periodic ticks. An output packet rejected before construction adds no construction sample; a packet constructed then rejected remains a measured attempt. These durations do not change scheduling or run validity. Empty histograms skip count-array merge/reset operations while retaining overflow accounting. Snapshot windows are immutable and shared with the output queue; any mutable copy for writer processing happens on the writer thread. Snapshot consumers requesting owned windows still receive their own copy.
