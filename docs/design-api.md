...
An SSH probe is traced step by step (`observer/ssh_trace.py`): config and keys, DNS, TCP, key exchange, authentication, session, script and close are each timestamped as they complete, and AsyncSSH's own debug account of that one connection is kept beside them. The report goes to the server log, and a failure names the step it interrupted. AsyncSSH's debug level is raised only while a probe runs, and what it adds is captured by a logger filter and dropped, so a recording's streams are not narrated and no console fills with it. The options step is itself broken down: each key loader AsyncSSH calls is timed, and the remainder is the config parse.

Client keys are loaded once per process (`observer/ssh_keys.py`), not once per connection. Parsing a private key is CPU work that runs under the GIL, so probes started together loaded their keys in series — four probes paid four loads, and a group verification timed out while still building options. A cached load is reused while the directory it came from is unchanged, concurrent first callers wait for the one load in progress, and each connection gets its own copy of the key pair. Loads with a passphrase or supplied certificates are not cached.
