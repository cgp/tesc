...
- Fixed: the Happy Eyeballs argument broke every front-end check with a 500 under uvloop, which uvicorn uses on Linux and macOS and which does not accept `happy_eyeballs_delay`. Addresses are now resolved and raced in `reachability._dial`, which works on any event loop; a test simulates uvloop's rejection.
- Fixed: two SSH probes starting together could fail with "partially initialized module 'asyncssh' has no attribute 'connect'". The off-loop import read `sys.modules`, where a module appears as soon as its import starts; it now keeps its own record of finished imports and otherwise waits on the import lock. A test with a deliberately slow module reproduces the old failure.
...
- Fixed: group verification timed out in the SSH options step. Each probe built its options twice (the trace's own build, then `connect`'s), and probes started together loaded their private keys one after another under the GIL. Options are now built once and `connect` is handed the loaded keys, and client keys are cached per process (`observer/ssh_keys.py`), reloaded when anything in their directory changes, with one load shared by concurrent callers. Locally, four simultaneous probes went from 90/180/270/360 ms in options to ~3 ms each after the first load.
- The options step of an SSH trace now lists each key loader's time and files, with the config parse as the remainder.
