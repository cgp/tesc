...
use metrix_plan::mix::Breakpoint;
use serde::Serialize;
...
pub(crate) async fn run(
    ...
    for (index, rate) in rates.iter().copied().enumerate() {
        ...
        let step = summarize(&last, rate, from_ms, to_ms, plan.duration);
        if let Some(output) = output {
            output.note(
                "breakpoint_step",
                Severity::Info,
...
fn summarize(report: &Report, rate: f64, from_ms: u64, to_ms: u64, duration: Duration) -> Step {
    ...
    Step {
        ...
        measured_seconds: duration.as_secs_f64(),
